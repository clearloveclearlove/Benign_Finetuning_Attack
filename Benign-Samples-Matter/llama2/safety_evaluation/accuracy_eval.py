...
def extract_answers(text):
    """
    Extracts answers appearing after #### in the given text.
    
    Args:
        text (str): The input text containing answers prefixed by ####.
...
def question_read(text_file):
    ...
    num = len(text_file)
    for i in range(num):
        dataset.append(text_file.iloc[i, 0])
    
    return dataset


def answer_read(text_file):
    dataset = []
    num = len(text_file)
    for i in range(num):
        dataset.append(text_file.iloc[i, 1])
...
def main(
    ...
    **kwargs
    ...
    ann = open(prompt_file).read().strip().split('\n')
    ann = [json.loads(a) for a in ann]

    import pandas as pd
    dataset = pd.DataFrame(ann)
    # print(dataset)
    # input()
    
    
    question_dataset = question_read(dataset)
    answer_dataset = answer_read(dataset)
    ...
    with torch.no_grad():
        ...
        for idx, chat in enumerate(chats[:100]):
            ...
            answer = extract_answers(output_text)

            gt = extract_answers(answer_dataset[idx])

            print(output_text)
            print(answer, gt)
            out.append(1 if answer == gt else 0)
